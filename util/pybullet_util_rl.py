...
def get_link_iso(robot, link_idx, client = None):
...
def get_link_vel(robot, link_idx, client = None):
    info = client.getLinkState(robot, link_idx, 1, 1)
    ret = np.zeros(6)
...
def set_link_damping(robot, link_id, lin_damping, ang_damping, client = None):
    for i in link_id:
...
def set_joint_friction(robot, joint_id, max_force=0, client = None):
    client.setJointMotorControlArray(robot, [*joint_id.values()],
                                client.VELOCITY_CONTROL,
                                forces=[max_force] * len(joint_id))


def draw_frame(pos, rot,  linewidth=5.0, text=None, client = None):
...
def draw_link_frame(robot, link_idx, linewidth=5.0, text=None, client = None):
    # This only works when the link has an visual element defined in the urdf file
    if text is not None:
        client.addUserDebugText(text, [0, 0, 0.02],
                           textColorRGB=[1, 0, 0],
                           textSize=1.5,
                           parentObjectUniqueId=robot,
                           parentLinkIndex=link_idx)
...
def set_motor_impedance(robot, joint_id, command, kp, kd, client = None):
...
def set_motor_impedance_single_pd(robot, joint_id, command, kp, kd, client = None):
    trq_applied = OrderedDict()
    for (joint_name, pos_des), (_, vel_des), (_, trq_des) in zip(
            command['joint_pos'].items(), command['joint_vel'].items(),
            command['joint_trq'].items()):
...
def set_motor_trq(robot, joint_id, trq_cmd, client = None):
    trq_applied = OrderedDict()
...
def set_motor_pos(robot, joint_id, pos_cmd, client = None):
...
def set_motor_pos_vel(robot, joint_id, pos_cmd, vel_cmd, client = None):
    pos_applied = OrderedDict()
...
def get_sensor_data(robot, joint_id, link_id, pos_basejoint_to_basecom,
                    rot_basejoint_to_basecom, client = None):
    """
    Parameters
    ----------
    joint_id (dict):
...
def get_camera_image(cam_target_pos, cam_dist, cam_yaw, cam_pitch, cam_roll,
                     fov, render_width, render_height, nearval, farval, client = None):
...
def is_key_triggered(keys, key, client = None):
    o = ord(key)
    if o in keys:
...
def set_config(robot, joint_id, link_id, base_pos, base_quat, joint_pos, client = None):
